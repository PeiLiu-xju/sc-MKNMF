...
data_dropout <- read.csv("data_celseq2.csv")
rownames(data_dropout) <- data_dropout[,1]
data_dropout <- data_dropout[,-1]
data_label <- read.csv("data_celseq2_label.csv")
...
m <- nrow(data_dropout)#gene 
n <- ncol(data_dropout)#cell
ag <- g_pnew
Ag <- ag
Dg <- diag(abs(apply(Ag,2,sum)))
Ug <- diag(m) - solve(sqrt(Dg)) %*% Ag %*% solve(sqrt(Dg))
ac <- c_pnew
Ac <- ac
Dc <- diag(abs(apply(Ac,2,sum)))
Uc <- diag(n) - solve(sqrt(Dc)) %*% Ac %*% solve(sqrt(Dc))
